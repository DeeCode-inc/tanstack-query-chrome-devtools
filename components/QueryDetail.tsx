  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <h2 className="sticky bg-white dark:bg-gray-900 px-4 pt-3 pb-1 top-0 z-10 text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">
        {activeTab === "queries" ? "Query Details" : "Mutation Details"}
      </h2>
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-4 py-1">
        <div className="border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400">
              {activeTab === "queries" ? "Query key:" : "Mutation key:"}
            </span>
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize border ${STATUS_THEMES[query.status].bg} ${STATUS_THEMES[query.status].text} ${STATUS_THEMES[query.status].border}`}
            >
              {query.status}
            </span>
          </div>
          <pre className="text-sm text-gray-900 dark:text-gray-100 whitespace-pre-wrap break-all font-mono">
            {stringifyWithBigInt(query.queryKey, 2)}
          </pre>
        </div>
        ...
        <p
          ...
          title={formatAbsoluteTime(query.dataUpdatedAt)}
        >
      ...
      <div className="px-4 py-3 space-y-4">
        ...
        <section>
          ...
          <div className="text-sm">
            <TreeView
              data={query.data}
              editable={!!sendSetData}
              onFieldChange={
                sendSetData
                  ? (path, value) => sendSetData(query.queryHash, path, value)
                  : undefined
              }
              onFieldDelete={
                sendDeleteData
                  ? (path) => sendDeleteData(query.queryHash, path)
                  : undefined
              }
              onArrayClear={
                sendClearArray
                  ? (path) => sendClearArray(query.queryHash, path)
                  : undefined
              }
            />
          </div>
