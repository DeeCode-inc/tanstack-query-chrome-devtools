...
export const QUERY_SORT_OPTIONS: readonly {
  value: QuerySortTerm;
  label: string;
}[] = [
...
export const MUTATION_SORT_OPTIONS: readonly {
  value: MutationSortTerm;
  label: string;
}[] = [
  { value: "status", label: "Sort by status" },
  { value: "lastUpdated", label: "Sort by last updated" },
...
function compareQueries(
  ...
  switch (term) {
    case "status": {
      const rankDiff =
        QUERY_STATUS_RANK[a.status] - QUERY_STATUS_RANK[b.status];
...
function compareMutations(
  a: MutationEntry,
  b: MutationEntry,
  term: MutationSortTerm,
): number {
  switch (term) {
    case "status": {
      const rankDiff =
        MUTATION_STATUS_RANK[a.status] - MUTATION_STATUS_RANK[b.status];
      if (rankDiff !== 0) return rankDiff;
